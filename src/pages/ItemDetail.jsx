...
export default function ItemDetail() {
  ...
  const isReporter = item && session && item.reporterSessionId === session.id
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      ...
      setItem(itemData)
      const itemMessages = getMessagesForItem(id)
  ...
  const handleSend = () => {
    ...
    try {
      ...
      const newMessage = addMessageToItem(item.id, message, session.id, senderType)
      ...
      // Reload messages
      const updatedMessages = getMessagesForItem(item.id)
  ...
  const handleMarkAsFound = async () => {
    ...
    try {
      const updatedItem = updateItemStatus(item.id, 'found', session.id)
      setItem(updatedItem)
  ...
  const handleRemoveReward = () => {
    ...
    try {
      const updatedItem = updateItem(item.id, { rewardAmount: null })
      setItem(updatedItem)
    } catch (error) {
  ...
  const performCloseCase = async () => {
    ...
    try {
      ...
      const updatedItem = updateItemStatus(item.id, 'closed', session.id)
      if (rewardGiven && item.rewardAmount) {
        updateItem(item.id, { rewardGiven: true })
      }
                  ...
                  {messages.length === 0 ? (
                    ...
                    <AnimatePresence>
                      {messages.map((msg, index) => {
                        const isMyMessage = msg.senderSessionId === session?.id
                        return (
                          <motion.div
                            key={msg.id}
