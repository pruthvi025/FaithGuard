...
export async function getFCMToken(sessionId, templeCode) {
  ...
  try {
    ...
    // Common error: permission denied or service worker not registered
    return null
  }
}
 ...
 */
export function getActiveTokensForTemple(templeCode) {
  // In a real implementation, this would query Firestore
  // where tokens are stored with sessionId, templeCode, and expiry
  // For this demo, we return the current token if it matches
  const storedToken = localStorage.getItem(TOKEN_STORAGE_KEY)
  const storedTempleCode = localStorage.getItem(TOKEN_TEMPLECODE_KEY)
...
/**
 * Store token in Firestore (for server-side notification sending)
 * In production, this would be called after getting the token
 */
export async function storeTokenInFirestore(token, sessionId, templeCode) {
  // This is a placeholder for production implementation
  // In production, you would:
  // 1. Create/update a document in Firestore
  // 2. Collection: 'fcm_tokens'
  // 3. Document ID: sessionId
  // 4. Fields: { token, templeCode, sessionId, createdAt, expiresAt }
  
  // For now, we're using localStorage which is session-bound
  // Server-side notification sending would require Firestore integration
  console.log('Token stored locally (Firestore integration needed for production)')
}
